...
### 使用websocket替代resetapi轮询,节省api调用,推送时效性更强(同时去掉防抖)
...
99%的401来自ip变化而不是被挤掉或者cookies过期(实测)

### 3.api限制

原项目api调用过于限制过于保守,但是7x24h调用这样限制确实合理

而websockets分支的调用更趋向于短时间内多次而不是全天均匀调用,理论不会429()

~~人家vrcx也没有限流只是退避照样好好的,不要了不要了~~

暂时取消429限制

### 4.

还有缓慢的验证3次才通知,本来30s调用一次时效性就不高

vrcapi确实有可能返回错误的信息  
(~~也不知道是不是错误的还是sb好友上了1s又下了~~)

可以采取短时间(如10s后)再主动调api而不是等90s凑够3次

这样好友可能上线2分钟才推送

不知道websocket会不会有这个问题(目前没发现,但是vrcx对好友下线貌似有类似的判断)
### 其他
